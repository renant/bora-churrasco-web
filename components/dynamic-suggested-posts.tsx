...
interface DynamicSuggestedPostsProps {
  excludeSlug?: string;
  count?: number;
}
...
export default async function DynamicSuggestedPosts({ excludeSlug, count = 3 }: DynamicSuggestedPostsProps) {
    ...
    <section className="w-full bg-white rounded-lg p-6 md:p-8 shadow-md border border-gray-200 mt-12">
      ...
      {posts.length > 0 ? (
        ...
        <div className="text-center py-8">
          <p className="text-gray-500">Carregando mais conteúdo...</p>
        </div>
      )}
    </section>
  );
